...
function authenticate_user($email, $password,$connection) {
    $s_email = functions\sanitize_string($email,$connection);
...
function add_user($data,$connection) {
    ...
    $first_name = functions\sanitize_string($data['first_name'], $connection);
    $last_name = functions\sanitize_string($data['last_name'], $connection);
    $email_address = functions\sanitize_string($data['email_address'], $connection);
    $drivers_license_id = functions\sanitize_string($data['drivers_license_id'], $connection);
    $gender = functions\sanitize_string($data['gender'], $connection);
    $password = encrypt_password($data['password'], $connection);
...
function update_user($data) {
    ...
    $query =
        "UPDATE $user_table SET
         ...
         WHERE id = $id";
    if (mysqli_query($connection, $query)) return true;
    error_log("Failed to update user: " . mysqli_error($connection));
    return false;
}
...
function delete_user($user_id){
    ...
    $num_rows = mysqli_num_rows($connection, $users_trips);
